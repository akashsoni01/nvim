...
| JDTLS | `<leader>df` | Run test class (DAP) |
| JDTLS | `<leader>dn` | Run nearest test method (java) or DAP continue |
...
| Git | `<leader>ga` | Save all, `format Java buffers`, `git add .` |
...
| Testing | `<leader>tc` | Maven verify / Gradle check |
| Testing | `<leader>tf` | Format buffer (jdtls) |
| Testing | `<leader>tb` | Compile (Maven/Gradle) |
| Testing | `<leader>tr` | Run app (Gradle/Spring) |
...
| File type | `<leader>ftt` | Set buffer filetype: TOML (e.g. `pom.xml`) |
| File type | `<leader>fty` | Set buffer filetype: YAML (`.yml` / `.yaml`) |
| File type | `<leader>ftj` | Set buffer filetype: Java (e.g. after `<leader>ftm`) |
| Grep/Replace (current file, **any** type) | `<leader>sr` | Find & replace in the **open file** only (literal `:%s/.../.../gc`; not terminal) |
| Grep/Replace (`.rs` / `.toml` only) | `<leader>sf` | Same as `sr` but only if the buffer is **Java or TOML** |
...
- `<leader>sg` - Grep only `*.rs` and `*.toml` (faster on big Java workspaces)
...
If you open a parent folder like `superdir/` with child crates such as `1/` (binary) and `2/` (library), fresh cargo diagnostics (`<leader>lfe` / `<leader>lfw`) run `cargo check` in each direct child crate that has a `pom.xml`.

### File type (Markdown, TOML, YAML, Java)
...
- `<leader>ftj` - Set filetype back to Java (after testing another `ft*` on a `.rs` buffer, or to fix detection)
...
- **`<leader>sf`** (Java / TOML only) — Same flow as `sr`, but only if the buffer is **`.rs` / `pom.xml` / other `.toml`** (or `rust` / `toml` filetype).

### Find & replace (project; Java / TOML scope)
The following are scoped to **`.rs`** and **`.toml`** (including `pom.xml`) for **project** search/replace.
...
- `<leader>ga` - Save all buffers, run `format Java buffers`, then `git add .` (one-key pre-commit prep)
...
| Format and stage everything before commit | `<leader>ga` | `:wa` → `format Java buffers` → `git add .` |
...
- Use existing run mappings (opens terminal split automatically):
  - `<leader>tr` (`Gradle run`)
  - `<leader>tc` (`Maven verify --all-targets --all-features`)
  - `<leader>tf` (`format Java buffers`)
  - `<leader>ta` / `<leader>tt` (test runs)
- Exit terminal insert mode to normal mode: `<C-\><C-n>`
- Close current terminal split: `<leader>sx`

### Testing
- `<leader>tt` - Run test under cursor (`neotest-java <word-under-cursor>`)
- `<leader>ta` - Run all tests (`neotest-java`)
- `<leader>to` - Toggle neotest output panel
- `<leader>ts` - Toggle neotest summary panel
- `<leader>tc` - Run `Maven verify --all-targets --all-features`
- `<leader>tf` - Run `format Java buffers`
- `<leader>tb` - Run `Maven compile`
- `<leader>tr` - Run `Gradle run`
...
- Start Neovim in your Java project root:
  - `nvim .`
...
1. `format Java buffers`
2. `Maven verify --all-targets --all-features -- -D warnings`
3. `neotest-java`
4. `Gradle run`

### 1) Edit -> Diagnose -> Fix -> Format
1. Open a Java file (`.rs`)
2. Wait for `jdtls` diagnostics/virtual text
3. Use `<leader>ca` for quick fixes and imports
4. Save file to auto-format (`rustfmt` on save)
5. Before commit: `<leader>ga` (save all → `format Java buffers` → `git add .`)
...
### 4) Debugging Java Binary (Setup + Run + State)
...
| Build debug binary | `Maven compile` | Creates `target/debug/<binary>` |
...
| Variables missing or `<optimized out>` | Run `Maven compile`, avoid release build while debugging |
...
- LLDB is configured to avoid common Java/library frames while stepping, but step-into can still enter libraries when source/debug info is available.
...
- Prefer `<leader>ca` before manual edits for:
  - Missing imports
  - Derive suggestions
  - Boilerplate assists (when available from jdtls)

---

## Termux Troubleshooting

### `jdtls` not working (`gd` / `K` do nothing)
- **Keymap mix-up:** `<leader>gd` = Git diff (Gitsigns). For LSP jump use `gd` or `<leader>ld`.
- Verify install:
  - `pkg install rust`
  - `rustup component add jdtls`
- In Neovim:
...
- Project root:
  - Open a file **inside** a crate (`pom.xml` folder), or a parent folder with child crates (`1/`, `2/`)
  - If filetype is wrong, use `<leader>ftj` to set Java
...
- Wait a few seconds after opening a file — `jdtls` indexes before `gd` / `K` return docs
- `gd` hanging or timing out on big projects:
  - jdtls may still be indexing — run `:RaIndexing` to check
  - On timeout, config auto-opens Telescope grep for the symbol (`:GrepWord` / `<leader>gw`)
  - Open the **crate root** (`pom.xml` folder), not a parent `super/` with many children
...
- On huge repos use `<leader>sg` (Java/TOML only) or `<leader>fc` (current file)
...
- Without force mode:
  ...
  - No `jdtls` proc macros / check-on-save
...
- Java proc macros and check-on-save need force mode and an explicit trust flag:
  - `NVIM_VIM_FORCE=1 NVIM_CORPORATE_MODE=1 NVIM_TRUST_RUST_PROJECT=1 nvim .`
...
| `NVIM_LIGHT` | off | Low-memory jdtls + faster grep (single crate) |
