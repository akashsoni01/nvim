...
`lazy.nvim` config for **Java** development: jdtls, neotest-java, Mason DAP bundles, Telescope, Git, 30 themes.

## Requirements
- **JDK 21+** on PATH (`java`, `javac`); set `JAVA_HOME` when needed
- **Maven or Gradle** project (`pom.xml` / `build.gradle`)
- Mason packages: `jdtls`, `java-debug-adapter`, `java-test`

```bash
bash ./scripts/setup-java-macos.sh   # or setup-java-linux.sh / setup-java-termux.sh
```

Inside Neovim: `:Mason` → install all three Java packages. One-time: `:NeotestJava setup`.

## Highlights
- Coral default + 15 color families — `<leader>ul` (30 themes)
- jdtls LSP + neotest-java + DAP test/debug
- Maven/Gradle build keys: `<leader>tb/tc/tr/tt`
- jdtls: `<leader>oi` organize imports, `<leader>df/dn` test class/method
- All shared features: Telescope grep, diagnostic nav, git workflow, vim-only, enterprise flags

## Docs
- [`JAVA_NEOVIM_CHEATSHEET.md`](./JAVA_NEOVIM_CHEATSHEET.md)
...
- Step-by-step vendoring guide: [`JAVA_NEOVIM_CHEATSHEET.md`](./JAVA_NEOVIM_CHEATSHEET.md) (section **Offline / Vendor Plugins**)
...
- Disabled without `NVIM_VIM_FORCE=1`:
  ...
  - `jdtls` proc macros and check-on-save
...
- Java project code execution also requires force mode and an explicit trust flag:
...
6. Before commit: `<leader>ga` (save all → `format Java buffers` → `git add .`)

## Big Table: Widely Used Commands

| Category | Command / Key | What it does | Beginner tip |
|---|---|---|---|
| Open Project | `nvim .` | Open Neovim in current folder | Run this from your Java project root |
...
| Find in `*.rs` / `*.toml` | `<leader>sg` | Telescope `live_grep` with `rg` globs | Project search only in Java and TOML files (needs `rg`) |
...
| Replace in `*.rs` / `*.toml` (buffer) | `<leader>sf` | Same as `sr`, scoped to Java/TOML buffers only | Use `sr` for other file types |
...
| Format Buffer | `<leader>fm` | Format current file | Javafmt also runs on save |
...
| Parent Java Folder | `nvim .` from parent | Check direct child crates like `1/` and `2/` | Used by `<leader>lfe` / `<leader>lfw` |
...
| Git Stage Prep | `<leader>ga` | Save all → `format Java buffers` → `git add .` | One-key pre-commit workflow |
...
| Run Test | `<leader>tt` | Run nearest Java test | Fast feedback loop |
...
| Cargo Fmt | `<leader>tf` | Run `format Java buffers` on the project | Format whole crate before commit |
| Cargo Run | `<leader>tr` | Run Java app | Quick manual verification |
...
| Toggle Inlay Hints | `<leader>uh` | Show/hide Java inlay hints | Reduce visual noise when needed |
...
| `NVIM_LIGHT` | `NVIM_LIGHT=1 nvim .` | Low-memory jdtls; skip `target/` in grep |
