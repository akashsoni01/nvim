...
`lazy.nvim` config for **Swift** development: SourceKit LSP, SPM, LLDB DAP, Telescope, Git, 30 themes.
...
`:SwiftNewProject` shows these steps in Neovim.
...
See [swift.org/install](https://www.swift.org/install/) for platform details.

## Highlights
- Coral default + 15 color families — `<leader>ul` (30 themes)
- SourceKit LSP + completion + snippets
- Treesitter `swift` + format on save for `*.swift`
- LLDB DAP for SPM executables (`.build/debug/<name>`)
- SPM keys: `<leader>tb` build, `<leader>tr` run, `<leader>tt` test filter, `<leader>tc` resolve
- All shared features: Telescope grep, diagnostic nav, git workflow, vim-only mode, enterprise flags

## Docs
- Full shortcuts: [`SWIFT_NEOVIM_CHEATSHEET.md`](./SWIFT_NEOVIM_CHEATSHEET.md)
...
- Step-by-step vendoring guide: [`SWIFT_NEOVIM_CHEATSHEET.md`](./SWIFT_NEOVIM_CHEATSHEET.md) (section **Offline / Vendor Plugins**)
...
- Disabled without `NVIM_VIM_FORCE=1`:
  ...
  - `sourcekit-lsp` proc macros and check-on-save
...
5. Save to format (`SourceKit format` runs on save)
6. Before commit: `<leader>ga` (save all → `format Swift buffers` → `git add .`)
...
| Find Swift files | `<leader>sg` | Telescope `live_grep` with `rg` globs | Project search only in Rust and TOML files (needs `rg`) |
...
| Jump to Definition | `gd` / `<leader>ld` | Jump to symbol definition | 12s timeout; Telescope grep fallback |
...
| Git Stage Prep | `<leader>ga` | Save all → `format Swift buffers` → `git add .` | One-key pre-commit workflow |
...
| Cargo Fmt | `<leader>tf` | Run `format Swift buffers` on the project | Format whole crate before commit |
...
| `NVIM_LIGHT` | `NVIM_LIGHT=1 nvim .` | Low-memory sourcekit-lsp; skip `target/` in grep |
| (N/A on Swift branch) | with force + corporate | Allow rust proc macros on trusted repos |
