-- Find/replace in current buffer or project (Swift: *.swift + Package.swift).
local M = {}
...
local GLOB_ARGS = { "-g", "*.swift", "-g", "Package.swift", "-g", "Package.resolved" }
...
function M.replace_in_file()
  if not project.buf_is_project_file() then
    vim.notify("Find/replace in file: use a .swift buffer or Package.swift.", vim.log.levels.WARN)
    return
  end
...
function M.find_in_project()
  if vim.fn.executable("rg") ~= 1 then
    vim.notify("ripgrep (rg) required. Scoped to Swift files.", vim.log.levels.ERROR)
    return
  end
  require("telescope.builtin").live_grep(
    telescope_grep.live_grep_opts({
      prompt_title = "Grep in Swift project",
...
function M.replace_in_project()
  ...
  vim.ui.input({ prompt = "Search in Swift files (literal): " }, function(search)
    if search == nil or search == "" then
      return
    end
    vim.ui.input({ prompt = "Replace with: " }, function(repl)
      ...
      replace_in_paths(paths, search, repl, " (*.swift / Package.swift)")
    end)
  end)
end
