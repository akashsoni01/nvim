...
local function java_compiler_messages(root, levels)
  vim.notify("Running compile check…", vim.log.levels.INFO)
  local result = vim.system({ "bash", "-lc", java_build.shell_compile() }, { cwd = root, text = true, stderr = true }):wait()
  local items = {}
  local seen = {}
  local combined = (result.stdout or "") .. "\n" .. (result.stderr or "")

  for line in combined:gmatch("[^\n]+") do
    local level, path, lnum, col, msg = line:match("%[(ERROR|WARNING)%] ([^:]+):%[(%d+),(%d+)%] (.+)$")
    if path then
      level = level:lower()
      if levels[level] then
        local filename = absolute_path(root, path)
        local key = filename .. lnum .. msg
...
local function collect_items(kind, opts)
  ...
  if opts.run_build ~= false then
    ...
    for _, root in ipairs(project.project_roots()) do
      local build_items = java_compiler_messages(root, levels)
...
function M.telescope_compile_issues(kind, opts)
  ...
  if #items == 0 then
    local source = opts.run_build == false and "current LSP diagnostics or cached compile results" or "compile/LSP diagnostics"
    ...
    .new({}, {
      ...
      previewer = previewers.new_buffer_previewer({
        ...
        define_preview = function(self, entry)
          ...
          vim.bo[self.state.bufnr].filetype = "java"
        end,
      }),
...
function M.jump_issue(direction, kind)
  ...
  local n = #items
  if n == 0 then
    vim.notify("No " .. kind .. "s found in current LSP diagnostics or cached compile results.", vim.log.levels.INFO)
    return
  end
