...
vim.api.nvim_create_user_command("FW", function()
  telescope_grep.find_word()
end, { desc = "Visual: find selection in buffer; Normal: find word in project" })
...
map({ "n", "v" }, "<leader>fW", function()
  telescope_grep.find_word()
end, vim.tbl_extend("force", opts, { desc = "Find word (visual: buffer, normal: project)" }))
map("v", "<leader>fr", function()
  rs_toml.rename_selection("buffer")
end, vim.tbl_extend("force", opts, { desc = "Rename selection in buffer (visual)" }))
map("v", "<leader>fR", function()
  rs_toml.rename_selection("project")
end, vim.tbl_extend("force", opts, { desc = "Rename selection in project (visual)" }))
